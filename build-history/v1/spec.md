...
This folder (`build-history/`) permanently retains the spec and decision log for every plan we build, versioned (`v1/`, `v2/`, …). Within a version, decisions are appended to `decisions.md`; supporting research goes in `appendix/` and is linked from the D-number. Future plans must write their spec here before or as they are built. See `build-history/README.md`.
