...
## D45. One source per writer, not per export file

An Apple Health export is a container. One file carries readings from the watch, the phone, a bathroom scale, a blood-pressure cuff, and any app that writes into Health — including WHOOP and Oura. The importer filed all of it under one source, `apple-1`. That broke D31 without any single record being wrong. Measured on a real export: a run on 2026-08-29 had 891 active-energy samples from the watch totalling 489 kcal and 37 from Oura totalling 477 kcal, and the file held them as 928 samples under one name — a sum of 966 kcal that no device produced. A scale's body weight, a cuff's blood pressure and the phone's step count were in the same bucket as the watch's heart rate.

**Each writer is its own source.** Every record in the export names who wrote it (`sourceName`), and that name is the identity. `Apple Watch`, `iPhone`, `Withings`, `Omron`, `Oura` and `WHOOP` inside one export become `apple-watch-1`, `iphone-1`, `withings-1`, `omron-1`, `oura-1` and `whoop-1`. A leading possessive is dropped from the id (`Alex's Apple Watch` → `apple-watch-1`), because a person's name does not belong in an identifier; the writer's name is kept as written in a new `writer` field. Points, series, sleep, workouts and beat lists all carry the writer of the record they came from, and two writers of the same quantity never share a sidecar.

Four sub-decisions, each settled against a real export rather than in the abstract:

- **A replaced watch that keeps its name stays one source.** Apple also writes a `device` attribute with a name, manufacturer, model and hardware string, and it would have been possible to split on the hardware. It would also have split a single watch: older records in the same export carry no `device` attribute at all, so splitting on it would put one watch in two sources — the pooling error inverted. The hardware is recorded instead, in a `devices` list on the source, without the software version or the memory address that change on their own. A watch replaced with the same name lists both.
- **The same writer arriving by two routes is two sources.** WHOOP's copy of its readings inside Apple Health is a rounded subset of WHOOP's own CSV export. A new `via` field names the route (`apple_health`, `whoop_csv`, `oura_csv`) and is part of the identity. This is the route-versus-device question D31 left open, answered the way D31 points: keep them apart and let the comparison be made.
- **Hand-typed readings go under the manual source.** The Health app names itself as the writer of a height someone typed in. It measured nothing, so those readings go to the file's `manual` source, which `init` already creates for exactly this.
- **A span sample keeps its length.** Steps, energy, distance and exercise time cover a stretch of time, and Apple's HRV figures describe a window. The end time had been dropped. It is now kept as a third parallel array, `durations_ms`, present only on days where a sample has a span. A total is a rate only if its length survives, and the span shows where a device stopped recording. The cost is roughly a third more bytes in those sidecars, paid once, at the moment everyone has to re-import anyway.

**Files in the old layout are refused, with the remedy.** Nothing in a pooled file says which reading came from which device — only the original export does — so it cannot be repaired in place, and importing on top of it would leave the pooled history beside the separated one, counted twice. An export source without a `writer` is the tell. The import stops and says to `ath init` in a new folder and import the exports again. The v0.1.0 fixture's sources are unaffected: they are `wearable`, not `export_file`.

Rejected alternatives:

- **Split on vendor, so `apple-1` holds every Apple device.** The phone in a pocket and the watch on a wrist measure different things, and Apple is the manufacturer of both. Vendor is kept as a field for readers grouping by maker; it is not the identity.
- **Split on the device fingerprint.** Rejected above: it breaks one watch into two on the records that lack the attribute, and splits again on every hardware generation.
- **Pool WHOOP-via-Apple with WHOOP-via-CSV, since it is one strap.** The two routes carry different data — the relay is a rounded subset — and pooling them would hide by how much. D31 says never merge across sources, and the route is part of what made these two.
- **Repair pooled files in place.** Not possible from the file. The export has to be read again, and a fresh folder is the only path that does not leave both histories side by side.

How the bug survived the tests: the fixture had a Withings scale and an Omron cuff in it from the start, each with its own `sourceName`, and every test passed with all of them under `apple-1`. The tests checked values and counts. None asked which source a reading was under, because the fixture and the code were written on the same assumption — that one export meant one source. Same lesson as D34 and D44: a test built on the code's assumption cannot catch that assumption being wrong. The tests now name the source for every reading they check.
