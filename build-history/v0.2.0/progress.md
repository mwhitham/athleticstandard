...
| 13 | Beat clocks placed by minute and second, hour discarded: D44 | ✅ done |
| 14 | One source per writer inside an export; devices recorded; spans kept; pooled files refused: D45 | ✅ done |

State as of 2026-09-06: v0.2.0 complete. 212 tests passing, up from 29.
...
- **The fix for that bug still lost 324,542 of 588,612 beats, and this time said so.** Apple writes a beat's hour against a different UTC offset than the record's own start time, so the two disagree by whole hours across daylight-saving boundaries and moves between zones — `10:33:19.09 PM` inside a window starting `23:33:17-05:00`. D42 had been tested against every spelling of a clock while sharing the parser's assumption that the two hours meant the same thing. The hour is now discarded and the minute and second are fitted to the window. See D44.
...
- **Every reading in an Apple export was filed under one source.** The watch, the phone, a Withings scale, an Omron cuff, Oura and WHOOP all shared `apple-1`, which is the pooling D31 forbids without any one record being wrong. On a real run, 891 watch calorie samples and 37 Oura calorie samples sat in one file as 928 samples totalling 966 kcal — a number no device produced. Each writer is now its own source, the hardware is recorded without splitting on it, the same writer by two routes is two sources, and a file in the old layout is refused with the remedy. Existing folders have to be rebuilt from the exports; the file cannot say which reading came from where. See D45.
