...
`result` is a **score object**: at least one of `duration_s` (for time-scored benchmarks), `reps`, `weight_kg`. The key must match the benchmark's `score_type` (validated). `scaling` is `rx` | `scaled`, optional.
